            ...
            <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md border border-white/20 p-3 rounded-2xl shadow-sm w-full md:w-auto">
              <div className="flex -space-x-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className={`w-10 h-10 rounded-full border-2 border-[#3B82F6] flex items-center justify-center font-bold text-sm ${i === 1 ? "bg-blue-100 text-blue-600" : i === 2 ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"}`}
                  >
                    D
                  </div>
                ))}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDoctors.map((doc, idx) => (
              <div
                key={idx}
                className="bg-white rounded-[24px] shadow-[0_2px_15px_-4px_rgba(0,0,0,0.05)] border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group"
              >
                {/* Top Banner Context */}
                <div
                  className={`h-2 w-full ${doc.waitingCount > 10 ? "bg-orange-400" : doc.waitingCount > 5 ? "bg-blue-400" : "bg-emerald-400"}`}
                ></div>
                ...
                <div className="p-6 md:p-8 flex flex-col h-full">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-[19px] font-bold text-slate-800 leading-tight mb-1">
                          {doc.user?.name}
                        </h3>
                  ...
                  <div className="mb-5 flex items-center justify-between bg-slate-50 px-4 py-3 rounded-xl border border-slate-100">
                    <div className="flex items-center gap-2">
                      <Activity
                        className={`w-4 h-4 ${doc.waitingCount > 10 ? "text-orange-500" : "text-emerald-500"}`}
                      />
                    ...
                    <div className="flex items-center gap-2">
                      {doc.waitingCount > 0 ? (
                        <>
                          <div
                            className={`w-2 h-2 rounded-full animate-pulse ${doc.waitingCount > 10 ? "bg-orange-500" : "bg-emerald-500"}`}
                          ></div>
                          <span
                            className={`text-sm font-bold ${doc.waitingCount > 10 ? "text-orange-600" : "text-emerald-600"}`}
                          >
