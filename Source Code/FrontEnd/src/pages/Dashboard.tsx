          ...
          <div className="max-w-7xl mx-auto w-full flex items-center justify-between">
            <div className="flex items-center gap-2 opacity-90 font-light">
              ...
              <span>
                ...
                · {user.name}{" "}
                <span className="bg-white/20 px-1.5 py-0.5 rounded text-xs ml-1 mr-1">
                  @{user.name.toLowerCase().split(" ")[0]}
                </span>{" "}
      ...
      <main className="max-w-[1200px] mx-auto px-4 py-8 space-y-8">
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-3 pt-1">
                <button
                  ...
                  className="bg-white/20 backdrop-blur-md border border-white/10 px-3.5 py-1.5 rounded-full flex items-center gap-2 text-[13px] font-medium shadow-sm hover:bg-white/30 transition-colors cursor-pointer"
                >
            ...
            <div className="flex gap-4 w-full md:w-auto mt-6 md:mt-0 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
              {[
                ...
                <div
                  key={i}
                  onClick={stat.action}
                  className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-[20px] p-4 flex flex-col items-center justify-center min-w-[90px] md:min-w-[105px] h-[105px] shadow-sm hover:bg-white/15 transition-colors cursor-pointer"
                >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
            {[
              ...
              <button
                ...
                className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] hover:shadow-md hover:border-slate-200 transition-all flex flex-col items-center justify-center gap-4 group"
              >
                <div
                  className={`${action.color} p-4 rounded-2xl shadow-sm group-hover:scale-110 transition-transform duration-300`}
                >
        ...
        <div className="bg-[#EF4444] rounded-[24px] p-6 shadow-md text-white flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <a
              href="tel:108"
              className="flex-1 md:flex-none flex items-center justify-center gap-2.5 bg-white text-[#EF4444] font-bold py-3.5 px-8 rounded-xl hover:bg-slate-50 transition-colors shadow-sm text-lg"
            >
              <Phone className="w-5 h-5" /> Call 108
            </a>
            <button
              onClick={() => navigate("/blood-bank")}
              className="flex-1 md:flex-none flex items-center justify-center gap-2.5 bg-[#DC2626] text-white font-bold py-3.5 px-8 rounded-xl hover:bg-red-800 transition-colors shadow-sm border border-red-400/30 text-lg"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-8" id="reminders-section">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => navigate("/health-records")}
                  className="text-[#3B82F6] text-[15px] font-medium hover:underline flex items-center gap-1"
                >
              ...
              {appointments.filter(
                ...
                <div className="space-y-3">
                  {appointments
                    ...
                    .map((appt: any, i) => (
                      <div
                        key={i}
                        className="bg-white rounded-2xl border border-slate-200 p-5 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow"
                      >
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          <span
                            className={`px-3 py-1 text-xs font-semibold rounded-full ${appt.status === "approved" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                          >
                ...
                <div className="bg-white rounded-[24px] border border-slate-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-8 flex flex-col items-center justify-center text-slate-400">
                  ...
                  <button
                    onClick={() => navigate("/appointments")}
                    className="mt-4 px-6 py-2 bg-blue-50 text-[#3B82F6] rounded-lg font-medium text-[13px] hover:bg-blue-100 transition-colors"
                  >
                    Book Now
                  </button>
                </div>
              ...
              {labBookings.length > 0 ? (
                <div className="space-y-3">
                  {labBookings.map((booking: any, i) => (
                    <div
                      key={i}
                      className="bg-white rounded-2xl border border-slate-200 p-5 flex items-center justify-between shadow-sm hover:border-[#A855F7]/30 transition-colors relative overflow-hidden"
                    >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-sm text-slate-500 mt-0.5">
                            ...
                            <span
                              className={`px-2 py-0.5 text-[11px] font-bold rounded-md uppercase ${booking.type === "home" ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-700"}`}
                            >
                              {booking.type === "home"
                                ? "Home Collect"
                                : "Walk-In"}
                ...
                <div className="bg-white rounded-[24px] border border-slate-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-8 flex flex-col items-center justify-center text-slate-400">
                  ...
                  <button
                    onClick={() => navigate("/labs")}
                    className="mt-4 px-6 py-2 bg-purple-50 text-[#A855F7] rounded-lg font-medium text-[13px] hover:bg-purple-100 transition-colors"
                  >
                    Book Lab Test
                  </button>
                </div>
          ...
          <div className="space-y-4 lg:pt-11">
            <div className="bg-white rounded-[24px] border border-slate-200 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] p-8 flex flex-col items-center text-center">
              ...
              <button
                onClick={() => navigate("/health-records")}
                className="mt-6 w-full py-3 border border-slate-200 rounded-xl text-[14.5px] font-semibold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-colors"
              >
                View Full Profile
              </button>
            </div>
